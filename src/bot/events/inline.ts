...
interface ExtendedInlineQueryResultVideo extends InlineQueryResultVideo {
  thumb_url: string;
}

export const handleInlineQuery = (bot: Telegraf<Context>) => {
  bot.on("inline_query", async (ctx: Context) => {
    ...
    if (urlMatch) {
      ...
      try {
        ...
        const videoPath: string = await downloadVideo(url, bot);
          ...
          [
            {
              ...
              thumb_url: "https://img.icons8.com/fluency/48/000000/video.png", // Миниатюра для видео
            } as ExtendedInlineQueryResultVideo,
          ],
          { cache_time: 1 }
        );
      } catch (error: any) {
    ...
    try {
      ...
      const apiKey: string | undefined = process.env.GPT_API_KEY;
      const folderId: string | undefined = process.env.FOLDER_ID;
      ...
      const textResponse: string = response.data.result.alternatives[0].message.text;
